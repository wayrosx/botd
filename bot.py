...
QUESTION, MODERATION, BROADCAST = range(3)  # Добавлено состояние BROADCAST
...
class MinecraftBot:
    ...
    def setup_handlers(self):
        ...
        conv_handler = ConversationHandler(
            ...
            states={
                ...
                # Добавлено состояние для рассылки
            },
    ...
    def admin_keyboard(self):
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("✉️ Уведомление", callback_data="admin_broadcast")],
    ...
    async def show_applications(self, query, context: ContextTypes.DEFAULT_TYPE):
        try:
            ...
            await query.edit_message_text(
                "❌ Произошла ошибка при загрузке заявок. База данных была пересоздана, попробуйте снова.",
                reply_markup=self.admin_keyboard()
            )
    ...
    async def show_questions(self, query, context: ContextTypes.DEFAULT_TYPE):
        try:
            ...
            await query.edit_message_text(
                "❌ Произошла ошибка при загрузке вопросов. База данных была пересоздана, попробуйте снова.",
                reply_markup=self.admin_keyboard()
            )
    ...
    async def handle_application_action(self, query, data, context: ContextTypes.DEFAULT_TYPE):
        ...
        if action == "review":
            ...
            try:
                await self.application.bot.send_message(
                    application['user_id'],
                    f"🎉 Ваша заявка на модерацию одобрена администратором {admin_name}! С вами свяжется администрация."
                )
            except Exception as e:
            ...
            applicant_name = f"@{applicant_info['username']}" if applicant_info and applicant_info.get(
                'username') else f"ID: {application['user_id']}"
    ...
    async def handle_question_action(self, query, data, context: ContextTypes.DEFAULT_TYPE):
        ...
        if action == "review":
            ...
            await query.edit_message_text(f"✅ Вопрос #{question_id} удален администратором {admin_name}.",
                                          reply_markup=self.admin_keyboard())
    ...
    async def start_broadcast(self, query, context: ContextTypes.DEFAULT_TYPE):
        ...
        await query.edit_message_text(
            ...
            "Или нажмите '❌ Отменить' для возврата в панель администратора.",
    ...
    async def receive_broadcast(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        for user in users:
            try:
                ...
                if not db.is_user_banned(user['user_id']):
                    await self.application.bot.send_message(user['user_id'], text)
                    success += 1
            except Exception as e:
                failed += 1
